...
<div class="bg-white rounded-2xl border border-[#E7E2DA] shadow-paper hover:shadow-paper-lg transition-all duration-200 overflow-hidden flex flex-col group">
    ...
    <div class="p-5 flex-1 flex flex-col justify-between space-y-4">
        <div>
            <h3 class="text-base font-bold text-zinc-900 line-clamp-1 group-hover:text-red-600 transition-colors">
                {{ $item->item_name }}
            </h3>
        ...
        <a href="{{ route('lost-items.show', $item->id) }}" class="w-full text-center py-2 px-4 rounded-xl bg-zinc-900 text-white text-xs font-semibold hover:bg-zinc-800 transition-colors block">
            Lihat Detail
        </a>
    </div>
</div>
