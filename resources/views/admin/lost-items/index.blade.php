<x-layout.admin header="Kelola Barang Hilang">
    ...
    <div class="space-y-6">
        ...
        <div class="bg-white p-4 rounded-2xl border border-[#E7E2DA] shadow-paper flex flex-col sm:flex-row items-center justify-between gap-4">
            <form action="{{ route('admin.lost-items.index') }}" method="GET" class="w-full sm:w-auto flex-1 flex flex-col sm:flex-row gap-3">
                <input type="text" name="q" value="{{ request('q') }}" placeholder="Cari barang atau nama pelapor..." class="px-4 py-2.5 text-xs bg-[#FAF8F4] border border-[#E7E2DA] rounded-xl focus:outline-none focus:border-zinc-900 flex-1">
                ...
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="bg-[#FAF8F4] border-b border-[#E7E2DA] text-[11px] font-bold text-zinc-500 uppercase tracking-wider">
                            ...
                            <th class="py-3.5 px-4">Pelapor & WA</th>
                            <th class="py-3.5 px-4">Tgl Hilang</th>
                    ...
                    <tbody class="divide-y divide-zinc-100 text-xs">
                        @forelse ($lostItems as $item)
                            <tr class="hover:bg-zinc-50 transition-colors">
                                ...
                                <td class="py-3.5 px-4 text-zinc-600">
                                    {{ \Carbon\Carbon::parse($item->lost_date)->translatedFormat('d M Y') }}
                                </td>
                                <td class="py-3.5 px-4">
                                    <x-business.status-badge :status="$item->status" />
                                </td>
                                <td class="py-3.5 px-4 text-right space-x-2">
                                    <a href="{{ route('admin.lost-items.show', $item->id) }}" class="px-2.5 py-1.5 rounded-lg bg-zinc-100 text-zinc-800 hover:bg-zinc-200 font-semibold text-[11px] inline-block">
                                        Detail / Match
                                    </a>
                        ...
                        @empty
                            <tr>
                                <td colspan="6" class="py-8 text-center text-zinc-400 text-xs">Belum ada data laporan barang hilang.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
