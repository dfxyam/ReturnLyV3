<x-layouts.admin title="Kelola Barang Hilang - ReturnLy">
    ...
    <div class="space-y-6">
        ...
        <div class="glass-card p-4 rounded-[24px] border border-white/10 bg-slate-900/40 backdrop-blur-xl flex flex-col sm:flex-row items-center justify-between gap-4">
            <form action="{{ route('admin.lost-items.index') }}" method="GET" class="w-full sm:w-auto flex-1 flex flex-col sm:flex-row gap-3">
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari barang atau nama pelapor..." class="px-4 py-2.5 text-xs bg-slate-950/60 border border-white/10 rounded-[16px] text-white placeholder-slate-500 focus:outline-none focus:border-emerald-500 flex-1">
                ...
                <table class="w-full text-left text-xs text-slate-300 border-collapse">
                    <thead class="bg-slate-950/80 text-slate-400 uppercase font-semibold text-[10px] tracking-wider border-b border-white/10">
                        <tr>
                            ...
                            <th class="py-4 px-6">Pelapor & WA</th>
                            <th class="py-4 px-6">Tgl Hilang</th>
                    ...
                    <tbody class="divide-y divide-white/5">
                        @forelse ($items as $item)
                            <tr class="hover:bg-white/5 transition-colors">
                                ...
                                <td class="py-4 px-6">
                                    <span class="font-semibold text-white block">{{ $item->reporter_name }} @if($item->class_name)({{ $item->class_name }})@endif</span>
                                    <span class="text-[11px] text-emerald-400 font-mono">{{ $item->phone_number }}</span>
                                </td>
                                <td class="py-4 px-6 text-slate-400 whitespace-nowrap">
                                    {{ $item->lost_date->translatedFormat('d M Y') }}
                                </td>
                                <td class="py-4 px-6 whitespace-nowrap">
                                    <x-business.status-badge :status="$item->status" />
                                </td>
                                <td class="py-4 px-6 text-right space-x-2 whitespace-nowrap">
                                    <a href="{{ route('admin.lost-items.show', $item->id) }}" class="px-3 py-1.5 rounded-[12px] bg-slate-800 text-slate-200 hover:text-white hover:bg-slate-700 font-semibold text-[11px] inline-block transition-all">
                                        Detail / Match
                                    </a>
                                    <form action="{{ route('admin.lost-items.destroy', $item->id) }}" method="POST" class="inline-block" onsubmit="return confirm('Yakin ingin menghapus data ini?')">
                                        @csrf
                                        @method('DELETE')
                        ...
                        @empty
                            <tr>
                                <td colspan="6" class="py-12 text-center text-slate-500 text-xs">Belum ada data laporan barang hilang.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
