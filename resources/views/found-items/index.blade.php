<x-layout.guest title="Daftar Barang Ditemukan - ReturnLy">

    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12">
        <!-- Header -->
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
            <div>
                <h1 class="text-2xl sm:text-3xl font-bold text-zinc-900 tracking-tight">Daftar Barang Ditemukan</h1>
                <p class="text-xs sm:text-sm text-zinc-600 mt-1">Lihat barang-barang yang diamankan di sekolah dan klaim barang milik Anda.</p>
            </div>
            <a href="{{ route('report.found') }}" class="inline-flex items-center justify-center px-4 py-2.5 bg-blue-600 hover:bg-blue-700 text-white font-bold text-xs rounded-xl shadow-sm transition-colors">
                + Lapor Penemuan
            </a>
        </div>
        ...
        <div class="bg-white p-4 rounded-2xl border border-[#E7E2DA] shadow-paper mb-8">
            <form action="{{ route('found-items.index') }}" method="GET" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
                <!-- Search Input -->
                <div class="relative">
                ...
                <div class="flex gap-2">
                    ...
                    @if (request()->hasAny(['q', 'category_id', 'location_id', 'status']))
                        <a href="{{ route('found-items.index') }}" class="px-3.5 py-2.5 bg-zinc-100 text-zinc-700 font-semibold text-xs rounded-xl hover:bg-zinc-200 transition-colors flex items-center justify-center" title="Reset Filter">
                            Reset
                        </a>
                    @endif
                </div>
        ...
        @if ($foundItems->isEmpty())
            <div class="text-center py-16 bg-white rounded-2xl border border-[#E7E2DA] p-6 shadow-paper">
                ...
                <h3 class="text-base font-bold text-zinc-900 mb-1">Tidak Ada Barang Ditemukan</h3>
                <p class="text-xs text-zinc-500 max-w-sm mx-auto mb-6">Coba ubah kata kunci atau filter pencarian Anda.</p>
                <a href="{{ route('found-items.index') }}" class="px-4 py-2 bg-zinc-900 text-white text-xs font-semibold rounded-xl">Lihat Semua Barang Ditemukan</a>
            </div>
        @else
